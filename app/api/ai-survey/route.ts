...
function readConfiguredProvider(): Provider | "" {
  const value = normalizeEnvValue(process.env.AI_PROVIDER).toLowerCase();
  if (value === "google") {
    return "gemini";
  }
...
function readOpenAiKey() {
  return readFirstEnv([process.env.OPENAI_API_KEY]);
}
...
function readAiGatewayKey() {
  return readFirstEnv([
    ...
    process.env.AI_SURVEY_API_KEY,
  ]);
}

function readGeminiKey() {
  return readFirstEnv([process.env.GEMINI_API_KEY, process.env.GOOGLE_API_KEY]);
}
...
async function callOpenAI(prompt: string, apiKey: string) {
  const model = normalizeEnvValue(process.env.OPENAI_MODEL) || "gpt-4o-mini";
    ...
    if (resp.status === 401 || resp.status === 403) {
      return {
        error: providerMessage
          ...
          : "OpenAI key huchingui esvel zovshoorolgui baina. `.env` dotorh `OPENAI_API_KEY`-ee shalgaad dev server-ee restart hiine uu.",
        status: resp.status,
      };
    }
...
async function callGemini(prompt: string, apiKey: string) {
  const model = normalizeEnvValue(process.env.GEMINI_MODEL) || "gemini-2.5-flash";
    ...
    if (resp.status === 401 || resp.status === 403) {
      return {
        error: providerMessage
          ...
          : "Gemini key huchingui esvel zovshoorolgui baina. `.env` dotorh `GEMINI_API_KEY`-ee shalgaad dev server-ee restart hiine uu.",
        status: resp.status,
      };
    }
...
async function callLovableGateway(prompt: string, apiKey: string) {
  ...
  if (!resp.ok) {
    ...
    if (resp.status === 401 || resp.status === 403) {
      return {
        error:
          "AI gateway key huchingui esvel zovshoorolgui baina. `.env` dotorh `LOVABLE_API_KEY`-ee shalgaad dev server-ee restart hiine uu.",
        status: resp.status,
      };
    }
...
export async function POST(req: NextRequest) {
  ...
  if (!providerConfig) {
    return NextResponse.json(
      {
        error:
          "AI provider tohiruulagdaagui baina. `.env` file-d `OPENAI_API_KEY`, `GEMINI_API_KEY`, esvel `LOVABLE_API_KEY` hiigeed dev server-ee restart hiine uu.",
      },
  ...
  try {
    ...
    return NextResponse.json({ survey: result.survey }, { headers: corsHeaders });
  } catch (error) {
